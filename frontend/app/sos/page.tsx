...
export default function SOSPage() {
  ...
  // Live clock — only starts after client mount to avoid hydration mismatch
  useEffect(() => {
    setMounted(true);
  ...
  const runAssessment = useCallback(() => {
    ...
    setAssessment(null);
    setTimeout(() => {
      const typeConfig = EMERGENCY_TYPES.find(e => e.id === selectedType);
  ...
  const accentColor = selectedType
    ? (EMERGENCY_TYPES.find(e => e.id === selectedType)?.color ?? "#FF4D6A")
    : "#FF4D6A";
      ...
      <div className="min-h-screen bg-[#080B0F] flex items-center justify-center p-6 font-mono">
        <div className="text-center max-w-md w-full">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5 text-left mb-6 space-y-3">
            {[
              ["Incident ID", `BR-SOS-${Date.now().toString().slice(-6)}`],
